...
function clearUbbcClasses(button) {
    ...
    classes.forEach(cls => {
        if (
            ...
            cls.startsWith("fl-txt-electric") ||
            cls.startsWith("fl-txt-peach") ||
            cls.startsWith("fl-txt-white") ||
            cls.startsWith("fl-bg-electric") ||
            cls.startsWith("fl-bg-sadsea") ||
            cls.startsWith("fl-bg-peach") ||
...
function getState(bib) {
    ...
    if (!lastClick && !lastCancel) return "prune";
...
function applyColor(button, state) {
    const styles = {
        prune: ["fl-bd-prune", "fl-txt-prune", "fl-bg-hov-prune","fl-bg-white","fl-txt-hov-white"],
        ...
        sadsea: ["fl-bd-sadsea", "fl-txt-sadsea", "fl-bg-hov-sadsea","fl-bg-white","fl-txt-hov-white"]
    };
...
function toggleBib(bib) {
    ...
    fetch(`toggle_lap.php?bib=${bib}`)
        ...
        .then(data => {
            const now = Date.now();
            const nowFormated = (new Date(now)).toLocaleTimeString('fr-FR', {
                hour12: false,
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit'
            ...
            document.getElementById("runner-name").innerHTML = `
                <h1 class="lead text-center fl-txt-prune fw-bold pt-2">${data.bib} • ${data.firstname.toUpperCase()} • ${data.race.toUpperCase()} • ${nowFormated}</h1>
            `;
