...
?>
  ...
  <section class="container-fluid">
        ...
        <!--<div class="panel-sub">    -->
            <h1 class="fl-txt-gray fl-txt-25 text-uppercase pt-2 text-center">RESULTS 2022</h1>
...
$sqlquery = <<< EOT
  ...
  FROM results l
  INNER JOIN
    (SELECT l.bib, @rank:=@rank+1 AS rank FROM results_2023 l,(SELECT @rank:=0) r ORDER BY laps DESC, chrono ASC) r
    ON r.bib=l.bib
  INNER JOIN
    (SELECT l.bib, @rank:=if(@gender=gender,@rank+1,1) AS rank, @gender:=gender FROM results l,(select @rank:=0, @gender:='') r ORDER BY gender ASC, laps DESC, chrono ASC) g
    ON g.bib=l.bib
  INNER JOIN
    (SELECT bib, @rank:=if(@category=category,@rank:=@rank+1,1) AS rank, @category:=category FROM results l,(SELECT @rank:=0, @category:='') r ORDER BY gender ASC, category ASC, laps DESC, chrono ASC) c
    ON c.bib=l.bib
