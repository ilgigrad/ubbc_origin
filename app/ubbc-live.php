...
$dt = new DateTime($record['current']);
              // $dt = new DateTime($record['current'], new DateTimeZone('UTC'));
             // $dt->setTimezone(new DateTimeZone('Europe/Paris'));
                $laststart_date = $dt->format('m-d');     // Ex. : 07-04
                $laststart_date = ""; //  on n'affiche pas la date
                $laststart_time = $dt->format('H:i:s');   // Ex. : 14:22:35
                if ($record["laps"]>=$record["goal"]){
                  $expected="finisher";
                  $expcolor="fl-txt-electric";
                }
                elseif ($record["expected"]=='99:59:59' || $record["control"]=='STOP'){
                    $expected="stopped";
                ...
                printf('<td class="medium"><span class="text-muted small">%s</span><span class="pl-3">%s</span></td>',$laststart_date,$laststart_time);
